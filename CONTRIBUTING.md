...
2. **Choose the appropriate division** — or propose a new one. Divisions are the
   ...
   > `scripts/convert.sh` and `scripts/lint-agents.sh`. Then regenerate the plugin
   > marketplace manifests — `python3 scripts/build-marketplace.py` — so the new
   > division is installable from the [plugin marketplace](integrations/plugin-marketplace/README.md).
...
- **Committed build output**: Generated files (`_site/`, compiled assets, converted agent files) should never be checked in. Users run `convert.sh` locally; its output is gitignored. When adding a new tool, adding that `.gitignore` rule is your step — see [Adding a Tool Integration](#adding-a-tool-integration). The one deliberate exception is the plugin marketplace manifests (`.claude-plugin/marketplace.json` and each `<division>/.claude-plugin/plugin.json`): they are generated by `scripts/build-marketplace.py` but **must be committed**, because marketplace installs clone the repo and run no build step. CI's `scripts/check-plugin-marketplace.sh` fails the build if they drift from the roster.
